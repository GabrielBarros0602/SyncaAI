...
- A task stores its start and its duration; ``end_at`` is generated by the database, which
  is legal because ``timestamptz + interval`` is ``IMMUTABLE`` (ADR-0008).
...
class Task(Base, TimestampMixin):
    ...
    scheduler assigns ``start_at`` (ADR-0008). ``end_at`` is generated by the database, so
    it cannot disagree with the two columns it derives from.
    ...
    end_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        Computed("start_at + duration_minutes * interval '1 minute'", persisted=True),
    )
